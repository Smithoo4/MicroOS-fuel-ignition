...
- `build.py`: The Python engine that decrypts variables in-memory, renders templates, and builds the ISO.
...
1. Decrypt `vars.yml` in-memory via SOPS (no plaintext written to disk).
2. Render the configuration files into the `output/` directory.
3. Generate a fresh `ignition.iso`.
